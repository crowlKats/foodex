...
- **Import from image**: upload photos of cookbook pages, handwritten notes, or
  ...
  translation.
- **Dictate a recipe**: press Dictate on New Recipe and talk the recipe
  through. The recording is transcribed into the box, where you can read it
  over and add instructions before the assistant turns it into a recipe. The
  assistant chat has the same button for spoken follow-ups.
...
OPENROUTER_API_KEY=...      # all model calls go through OpenRouter (openrouter/auto-beta; dictation pins Gemini Flash)
S3_BUCKET=...
S3_REGION=...
S3_ACCESS_KEY_ID=...
